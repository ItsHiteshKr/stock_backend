# Stock_Backend
this is for Stock_Price &amp; because  

.env Configuration 

MAIL_USERNAME= your email id
MAIL_PASSWORD= app password not your email password
MAIL_FROM= Your email
MAIL_FROM_NAME= Your name

MAIL_PORT=587
MAIL_SERVER=smtp.gmail.com
MAIL_STARTTLS=True
MAIL_SSL_TLS=False

Frontend_URL=


DB_USER=root
DB_PASSWORD=
DB_HOST=
DB_PORT=
DB_NAME=




DATABASE_URL=mysql+pymysql://<DB_USER><DB_PASSWORD>@<DB_HOST>t:<DB_PORT>/<DB_NAME>


JWT_SECRET_KEY = 
JWT_REFRESH_SECRET_KEY = 
JWT_ALGORITHM = HS256

ACCESS_TOKEN_EXPIRY_MINUTES = 30
REFRESH_TOKEN_EXPIRE_DAYS = 7

ADMIN_USERNAME =
ADMIN_PASSWORD =




# How to Update the Fundamentals Table

After starting the main FastAPI server, run the following command once (in new cmd) to fetch and store all fundamental data into the database:

python -m service.update_fundamentals_live


This script:

Downloads live stock fundamentals

Updates the MySQL/PostgreSQL database

Fills the fundamentals table automatically

# Make sure your .env file is configured before running it.