...
from schema.watchlist_schema import WatchlistCreate, WatchlistUpdate, WatchlistResponse
...
@router.get("/user/{email}", response_model=List[WatchlistResponse])
def get_user_watchlists(email: EmailStr, db: Session = Depends(get_db)):
    """Get all watchlists for a specific user"""
    return WatchlistService.get_user_watchlists(email, db)
...
@router.post("/{watchlist_id}/add-symbol/{symbol}", response_model=WatchlistResponse)
...
@router.delete("/{watchlist_id}/remove-symbol/{symbol}", response_model=WatchlistResponse)
